...
from scapy.contrib.pfcp import IE_NodeId, IE_FSEID, IE_CreatePDR, IE_PDR_Id, IE_Precedence, IE_PDI, IE_SourceInterface, \
    IE_FTEID, IE_SDF_Filter, IE_QFI, IE_OuterHeaderRemoval, \
    IE_CreateFAR, IE_FAR_Id, IE_ApplyAction, IE_ForwardingParameters, IE_DestinationInterface, \
    IE_Create_BAR, IE_BAR_Id, IE_CreateQER, IE_QER_Id, IE_GateStatus, IE_MBR, \
    IE_CPFunctionFeatures, IE_Cause, IE_UPFunctionFeatures, \
...
from scapy.contrib.pfcp import PFCP, PFCPHeartbeatRequest, PFCPHeartbeatResponse, \
    ...
    PFCPSessionEstablishmentRequest, PFCPSessionEstablishmentResponse, PFCPSessionModificationResponse, \
...
class PfcpSkeletonCopy(threading.Thread):
    teid_counter = 0
    seq = 1
    context = collections.defaultdict(tuple)
    pfcp_cp_ip = None
    response_task = {}
    smf_ip = "127.0.0.1"
    LOCAL_IE_FLAG = 1
    ...
    def __init__(self, sk, upf_ip, upf_gw):
        super().__init__()
    ...
    def check_local_seid_report(self, type):
        if "seid_support" in self.response_task[type][0] and self.response_task[type][0]["seid_support"]:
            self.response_task[type][0]["seid_support"] = 0
            return True
        return False

    def get_seid_to_main(self):
        return self.rsp_list.get()
    ...
    def set_rsp_task(self, response):
        if response['msg_type'] == PfcpMsgEnum.session_establishment_response.name:
            self.init_rsp_task(PfcpMsgEnum.session_establishment_response.name)
            self.set_task(response)
        if response['msg_type'] == PfcpMsgEnum.session_modification_response.name:
            self.init_rsp_task(PfcpMsgEnum.session_modification_response.name)
            self.set_task(response)

    def set_task(self, response):
        # TODO 新任务加在这里
        if "seid_support" in response:
            self.response_task[response['msg_type']][0]["seid_support"] = 1
            self.__logger.info("set_task 设置%s的seid回调", response["msg_type"])
        if "ue_ip" in response and is_ipv4_legal(response["ue_ip"]):
            # 特定IP添加前要再初始化
            self.init_rsp_ip_task(response)
            self.response_task[response['msg_type']][0][response["ue_ip"]].put(response['IEs_dict'])
            self.__logger.info("set_task 设置特定ue_ip，地址为%s", response["ue_ip"])
        else:
            self.response_task[response['msg_type']][1].put(response['IEs_dict'])
            self.__logger.info("set_task 设置消息%s的回调任务", response["msg_type"])

    def reset_rsp_task(self):
        self.response_task = {}

    # 获取任务
    def get_task_ie_list(self, type, ue_ip=None):
        if type not in self.response_task:
            return []
        if ue_ip and ue_ip in self.response_task[type][0]:
            return self.response_task[type][0][ue_ip].get_nowait()
        else:
            return self.response_task[type][1].get_nowait()

    # 通用消息回复队列上限10
    def init_rsp_task(self, type):
        if type not in self.response_task:
            # 回复类型下的二元元组，包括ip队列和通用队列
            self.response_task[type] = (dict(), Queue(maxsize=10))

    def init_rsp_ip_task(self, response):
        if response["ue_ip"] not in self.response_task[response['msg_type']][0]:
            self.response_task[response['msg_type']][0][response["ue_ip"]] = Queue(maxsize=5)
    ...
    def session_establish_response(self, pfcp, seq=1):
        ...
        if local_seid != 0:
            remote_seid, local_teid = self.context[local_seid]
        else:
        ...
        spec_ie_list = build_pfcp_session_establishment_response(
            self.get_task_ie_list(type_desc, ue_ipv4)
        )
        ...
        if self.check_local_seid_report(type_desc):
            self.rsp_list.put(local_seid)
            print("upf上报seid", local_seid)
    ...
    def session_modification_response(self, local_seid, seq=1):
        ...
        spec_ie_list = build_pfcp_session_modification_response(
            self.get_task_ie_list(type_desc)
        )

        self.chat(PFCPSessionModificationResponse(
            IE_list=spec_ie_list if spec_ie_list != [] else d4_ie_list
        ), seq=seq, seid=self.context[local_seid][0])
    ...
    def session_deletion_response(self, local_seid, seq=1):
        ...
        exist_flag = local_seid in self.context
        ie_list = not_found_list if not exist_flag else d4_ie_list
        self.chat(PFCPSessionDeletionResponse(
            IE_list=ie_list
        ), seq=seq, seid=self.context[local_seid][0] if exist_flag else None)
    ...
    def session_report_response(self, local_seid, seq=1):
        self.chat(PFCPSessionReportResponse(IE_list=[
            IE_Cause(cause=CauseValues[1]),
        ]), seq, seid=self.context[local_seid]['remote_seid'])
